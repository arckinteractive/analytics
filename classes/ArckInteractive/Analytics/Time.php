...
class Time {
	...
	/**
	 * Returns a timestamp for the start of the full hour
	 *
	 * @param mixed  $ts     Date/time value
	 * @param string $format Format of the return value
	 * @return string
	 */
	public static function getHourStart($ts = 'now', $format = 'U', $tz = null) {
		if (!self::isValidTimezone($tz)) {
			$tz = self::getClientTimezone();
		}
	...
	/**
	 * Returns a timestamp for the end of the full hour
	 *
	 * @param mixed  $ts     Date/time value
	 * @param string $format Format of the return value
	 * @return string
	 */
	public static function getHourEnd($ts = 'now', $format = 'U', $tz = null) {
		if (!self::isValidTimezone($tz)) {
			$tz = self::getClientTimezone();
		}
	...
	/**
	 * Returns a timestamp for the first of the year at 0:00:00
	 *
	 * @param mixed  $ts     Date/time value
	 * @param string $format Format of the return value
	 * @return string
	 */
	public static function getYearStart($ts = 'now', $format = 'U', $tz = null) {
		if (!self::isValidTimezone($tz)) {
			$tz = self::getClientTimezone();
		}
	...
	/**
	 * Returns a timestamp for the last day of theyear at 23:59:59
	 *
	 * @param mixed  $ts     Date/time value
	 * @param string $format Format of the return value
	 * @return string
	 */
	public static function getYearEnd($ts = 'now', $format = 'U', $tz = null) {
		if (!self::isValidTimezone($tz)) {
			$tz = self::getClientTimezone();
		}
	...
	public static function getTimeOfDay($ts_time = 0, $ts_day = null, $format = 'U', $tz = null) {
		...
		$time = self::getTime($ts_time, 'U', $tz);
		$day_start = self::getDayStart($ts_day, 'U', $tz);
	...
	function getRepeatFrequencies() {
		return array(
			self::FREQUENCY_DAILY => elgg_echo('events_ui:repeat:daily'),
			self::FREQUENCY_WEEKDAY => elgg_echo('events_ui:repeat:weekday'),
			self::FREQUENCY_WEEKDAY_ODD => elgg_echo('events_ui:repeat:dailymwf'),
			self::FREQUENCY_WEEKDAY_EVEN => elgg_echo('events_ui:repeat:dailytt'),
			self::FREQUENCY_WEEKLY => elgg_echo('events_ui:repeat:weekly'),
			self::FREQUENCY_MONTHLY => elgg_echo('events_ui:repeat:monthly'),
			self::FREQUENCY_YEARLY => elgg_echo('events_ui:repeat:yearly'),
		);
	}
	...
	/**
	 * Convert seconds to a duration in 0:00:00 format
	 * 
	 * @param int $seconds Number of seconds
	 * @return string
	 */
	public static function toDuration($seconds) {

		$hours = floor($seconds / self::SECONDS_IN_AN_HOUR);
		$seconds -= $hours * self::SECONDS_IN_AN_HOUR;

		$minutes = floor($seconds / self::SECONDS_IN_A_MINUTE);
		$seconds -= $minutes * self::SECONDS_IN_A_MINUTE;
